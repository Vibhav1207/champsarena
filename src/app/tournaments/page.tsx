...
// Pre-optimized sprite/placeholder images
const BANNER_PLACEHOLDER = "data:image/jpeg;base64,";
const FORMAT_ICON_PLACEHOLDER = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8/5+hHgAHggJ/PchI7wAAAABJRU5ErkJggg==";

function TournamentCard({ tournament }: { tournament: Tournament }) {
  const gameInfo = getTournamentGameInfo(tournament);
  const stat = STATUS_LABEL[tournament.status] || { label: tournament.status, color: "text-primary/70" };
  const used = tournament._count?.registrations ?? 0;
  const cap = Math.min(100, Math.round((used / tournament.maxPlayers) * 100));
  const isGoldBorder = tournament.entryFee > 25;
  ...
  return (
    <article className="bg-white border-4 border-primary neo-brutalist-shadow-hover transition-all flex flex-col">
      <div className="relative h-48 border-b-4 border-primary bg-surface-dim">
        <Image
          src={tournament.banner || gameInfo.banner || BANNER_FALLBACK}
        ...
        <div>
          ...
          <div className="grid grid-cols-1 gap-y-xs mb-md font-bold uppercase text-body-md text-primary">
            ...
            <div className="flex items-center gap-xs text-accent-red">
              ...
              <span>{currencySymbol}{tournament.prizePool.toLocaleString()} Pool</span>
            </div>
            <div className="flex items-center gap-xs">
              <span className="material-symbols-outlined text-[18px]">group</span>
              <span>{tournament.maxPlayers - used} / {tournament.maxPlayers} Slots Left</span>
            </div>
          </div>
        </div>
...
function FilterSidebar({ selectedFormats, setSelectedFormats, selectedStatus, setSelectedStatus, startDate, setStartDate, endDate, setEndDate, handleReset }: {
      ...
      <div className="bg-white p-md border-4 border-primary neo-brutalist-shadow-sm sticky top-[4px] sm:top-[120px]">
        ...
        <div className="mb-md mt-sm select-none">
          ...
          <div className="space-y-sm">
            {formatOptions.map(({ label, key, img, color }) => {
                ...
                >
                  <div className={`relative h-full w-20 shrink-0 ${color} border-r-2 border-primary overflow-hidden`}>
                    <Image
                      ...
                      quality={85}
                      placeholder="blur"
                      blurDataURL={FORMAT_ICON_PLACEHOLDER}
                    />
                  </div>
      ...
      <div className="flex gap-x-1">
        {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
          <button
            ...
            className={`w-12 h-12 flex items-center justify-center border-4 border-primary ${currentPage === page
              ? 'bg-black text-white'
...
export default function Tournaments() {
  ...
  }, [tournaments, selectedFormats, selectedStatus, startDate, endDate]);
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-3 gap-md">
                ...
                {paginatedFiltered.length > 0 ? (
                  ...
                  <div className="col-span-full flex flex-col items-center justify-center py-32 text-center gap-4 bg-white border-4 border-dashed border-primary">
                    <span className="material-symbols-outlined text-7xl text-7xl text-primary/30">emoji_events</span>
